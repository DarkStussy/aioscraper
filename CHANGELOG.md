...
- An adaptive rate limit outcome goes to the group that paced the request. The group key was derived again when the request finished, from the request object as the middleware chain left it, so a middleware rewriting what the grouping reads — a host, a proxy — tuned another group, or none at all when that key had no group. `Attempt` now carries the key it was routed under, and `group_by` is called once per attempt instead of twice.
- A request is no longer queued to a rate limit group whose worker has stopped. The manager learns that a group is gone from the worker task's done callback, which runs a tick after the worker itself ends, and anything queued in that window sat in the group's queue with nothing left to read it. The group is now replaced when its worker is no longer alive.
